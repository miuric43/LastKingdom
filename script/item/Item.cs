    ...
    public class Item
    {
      ...
      public virtual bool Stackable { get { return false; } }
        ...
        public virtual string ParsedStats
        {
            get
            {
                ...
                if (DamBase != 0)
                    ret += "파괴: " + DamBase + "\n\t";
                if (DamBase != 0)
                    ret += "파괴증가량: " + (FullDam - DamBase) + "\n\t";
                if (ACBase != 0)
                    ret += "방어: " + ACBase + "\n\t";
                if (ACBase != 0)
                    ret += "방어증가량: " + (FullAC - ACBase) + "\n\t";
                if (Quantity != 0)
                    ret += "작업: " + (Quantity - 1) + "\n\t";
                if (HitBonus != 0)
                    ret += "적중: " + Hitb + "\n\t";
                ...
                if (VitBonus != 0)
                    ret += "보너스생명: " + vitb + "\n\t";
                if (ReduceCast != 0)
                    ret += "캐스팅속도: " + (float)(ReduceCast * 0.01) + "\n\t";
                if (ReduceSwing != 0)
                    ret += "공격속도: " + (float)(ReduceSwing * 0.01) + "\n\t";
                if (Stage != 0)
                    ret += "강화레벨: " + Stage + "\n\t";
                if (ClassReq != 0)
                    ret += "필요직업: " + ClassReqString + "\n\t";
                if (LevelReq != 0)
                    ret += "필요레벨: " + lvlr + "\n\t";
                if (FlavorText != null)
                    ret += "\n\t  " + FlavorText + "\n\t";
       ...
       //     DropTime = LKCamelot.Server.tickcount.ElapsedMilliseconds;
        ...
        public bool TryUpgrade3()
        {
            if (Quantity == 101)
            {
                if (Util.Random(100, 100) >= 100 * (Stage * 0.1))
                {
                    Quantity = 201;
        ...
        public bool TryUpgrade4()
        {
            if (Quantity < 5000)
            {
                if (Util.Random(100000, 100000) >= 100 * (0 * 0.1))
                {
                    Quantity++;

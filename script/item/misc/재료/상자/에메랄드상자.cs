...
{
    public class 에메랄드상자 : BasePotion
    {
        public override string Name { get { return "에메랄드상자" + "     "; } }
        ...
        public override void Use(Player player)
        {
            if (Quantity >= 12 && player.GetFreeSlots() > 2)
              {
                  var newitem = new script.item.에메랄드원석().Inventory(player);
            ...
            else if (Quantity >= 3 && Quantity <= 11)
            {
                ...
                string itdrop = "에메랄드원석이" + (Quantity - 1) + "개남았습니다" + "                                          ";

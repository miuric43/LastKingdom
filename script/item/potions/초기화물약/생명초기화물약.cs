...
{
    public class 힘초기화물약 : BasePotion
    {
        public override string Name { get { return "StrReset Drug"; } }
        ...
        public override void Use(Player player)
        {
            ...
            if (player.GetFreeSlots() > 5 && player.Level <= 101 && Quantity >= 12 && player.m_Str >= 500)
                {
            ...
            else if (Quantity >= 3 && Quantity <= 11 && player.GetFreeSlots() > 5 && player.Level <= 101 && player.m_Str >= 500)
             {
                   foreach (var it in player.Equipped2.Values)
                    {
                        it.Unequip(player, it.InvSlot);
                    ...
                    player.client.SendPacket(new UpdateCharStats(player).Compile());
                    Quantity = Quantity - 1;
                    string itdrop = "힘초기화물약이" + (Quantity - 1) + "개남았습니다" + "                                          ";
                    player.client.SendPacket(new UpdateChatBox(0xff, 0xff, 1, (short)itdrop.Count(), itdrop).Compile());

                }
            else if (Quantity <= 3 && player.GetFreeSlots() > 5 && player.Level <= 101 && player.m_Str >= 500)
            {
               foreach (var it in player.Equipped2.Values)
                    {
                        it.Unequip(player, it.InvSlot);

            ...
            if (World.NewMonsters == null)
            {
                World.NewMonsters = new Dictionary<Serial, script.monster.Monster>()
                {
                    ...
                    { temps, new script.monster.SD(temps++, 6,14,"Rest")},
                       ...
                       //운영자맵 외벽 좌 
                       { temps, new script.monster.성기사(temps++, 20,20,"레이드")},
                          { temps, new script.monster.건물(temps++, 2,3,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 2,5,"GM존")},                     
                          { temps, new script.monster.건물(temps++, 2,7,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 2,9,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 2,11,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 2,13,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 2,15,"GM존")},
                          { temps, new script.monster.건물(temps++, 2,17,"GM존")},
                       
                          //운영자맵 외벽 우 
                          { temps, new script.monster.건물(temps++, 17,3,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 17,5,"GM존")},                     
                          { temps, new script.monster.건물(temps++, 17,7,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 17,9,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 17,11,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 17,13,"GM존")},                       
                          { temps, new script.monster.건물(temps++, 17,15,"GM존")},
                          { temps, new script.monster.건물(temps++, 17,17,"GM존")},

                          //운영자맵 외벽 하
                          { temps, new script.monster.건물(temps++, 3,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 4,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 5,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 6,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 7,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 8,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 9,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 10,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 11,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 12,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 13,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 14,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 15,17,"GM존")},                        
                          { temps, new script.monster.건물(temps++, 16,17,"GM존")},                        
                         //공성전
                         { temps, new script.monster.성물(temps++, 99,58,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 96,63,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 97,63,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 98,63,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 99,63,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 100,63,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 101,63,"공성맵")},

                          { temps, new script.monster.성벽(temps++, 96,64,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 97,64,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 98,64,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 99,64,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 100,64,"공성맵")},
                         { temps, new script.monster.성벽(temps++, 101,64,"공성맵")},
                ...
                for (int x = 0; x < 8; x++)
                {
                    ...
                    while ((tempmob = Map.SpawnPoint("튜토리얼")) == null) ;
                    World.NewMonsters.Add(temps, new script.monster.튜토(temps++, tempmob.X, tempmob.Y, "튜토리얼"));
                }
                for (int x = 0; x < 50; x++)
                {
                    Point2D tempmob = null;
                    while ((tempmob = Map.SpawnPoint("Beginner")) == null) ;
                    World.NewMonsters.Add(temps, new script.monster.WildDog(temps++, tempmob.X, tempmob.Y, "Beginner"));
                }
                for (int x = 0; x < 30; x++)
                {
                    Point2D tempmob = null;
                ...
                for (int x = 0; x < 20; x++)
                {
                    Point2D tempmob = null; string mapn = "바이킹";
                    while ((tempmob = Map.SpawnPointFixed(mapn, 0, 0, 51, 28)) == null) ;
                    World.NewMonsters.Add(temps, new script.monster.vViking(temps++, tempmob.X, tempmob.Y, mapn));
                }
                for (int x = 0; x < 20; x++)
                {
                    Point2D tempmob = null; string mapn = "바이킹";
                    while ((tempmob = Map.SpawnPointFixed(mapn, 0, 0, 21, 28)) == null) ;
                    World.NewMonsters.Add(temps, new script.monster.vButcher(temps++, tempmob.X, tempmob.Y, mapn));
                }
                for (int x = 0; x < 20; x++)
                {
                    Point2D tempmob = null;
                    while ((tempmob = Map.SpawnPointFixed("메두사", 0, 0, 99, 79)) == null) ;
                    World.NewMonsters.Add(temps, new script.monster.Medusa(temps++, tempmob.X, tempmob.Y, "메두사"));
                }
                for (int x = 0; x < 40; x++)
                {
                    Point2D tempmob = null;
                ...
                {
                    Point2D tempmob = null;
                    while ((tempmob = Map.SpawnPointFixed("Rest", 0, 0, 15, 15)) == null) ;
                    World.NewMonsters.Add(temps, new script.monster.Medusa(temps++, tempmob.X, tempmob.Y, "Rest"));

                    Console.WriteLine("테스트");
                    var message = "테스트" + "             ";
                    World.SendToAll(new QueDele("all", new UpdateChatBox(0x08, 0x02, 0, (short)message.Count(), message).Compile()));
                }
                */
            }
        }
